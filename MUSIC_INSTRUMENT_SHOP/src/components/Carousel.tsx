import React, { useRef } from "react"


const Carousel = () => {

  const sliderRef = useRef(null);

  function onClickHandle(e:any) {
    const slider:any = sliderRef.current;
    if(!slider) return;

    const sliderIndex = parseInt(getComputedStyle(slider).getPropertyValue("--slider-index")) || 0;

    if (e.target.classList.contains(".left-handle")) {
      slider.style.setProperty("--slider-index", sliderIndex - 1)
    }
    else if (e.target.classList.contains(".left-handle")) {
      slider.style.setProperty("--slider-index", sliderIndex + 1)
    }
  }


  return (
    <div className="container" onClick={onClickHandle}>
      <button className="handle left-handle" >
        <div className="text">&#129088;</div>
      </button>
      <div ref={sliderRef} className='slider'>
        <img src="https://thumbs.static-thomann.de/thumb//txteaser1000--16bcb21d2cf8859a60079edf7f73a7d6/pics/cms/image/teasertool/de/13242/448687_homepianos.webp?d=%3D" alt="alternative1" />
        <img src="https://thumbs.static-thomann.de/thumb//txteaser1000--b8eb7ab01cf6b54ff98965db6febe35a/pics/cms/image/teasertool/de/13242/universal_audio_topsellers.webp?d=%3D" alt="alternative2" />
        <img src="https://thumbs.static-thomann.de/thumb//txteaser1000--fe3a30e4bd6eabb5f450d21011dbf6b9/pics/cms/image/teasertool/de/13242/alesis_electronic_drums.webp?d=%3D" alt="alternative3" />
        <img src="https://thumbs.static-thomann.de/thumb//txteaser1000--eeb5ccaa2d9a81c100825310543913c9/pics/cms/image/teasertool/de/13242/aucoustic_guitars_topseller.webp?d=%3D" alt="alternative4" />
        <img src="https://thumbs.static-thomann.de/thumb//txteaser1000--d0ad3dcabd17e5443b4f07d77d48512c/pics/cms/image/teasertool/de/13242/bluetooth_speaker_topseller.webp?d=" alt="alternative5" />
        <img src="https://i.ytimg.com/vi_webp/435Ho_UEvcI/hqdefault.webp" alt="..." />
        <img src="https://www.thomann.de/blog/wp-content/uploads/2024/11/2411_Thomann_Dokumentation_Promo_Newsletter_770x425_EN-472x260.jpg" alt="..." />
        <img src="https://i.ytimg.com/vi_webp/HmIVXgNeuFo/hqdefault.webp" alt="..." />
        <img src="https://www.thomann.de/blog/wp-content/uploads/2025/01/Looper_header_770x425-2-472x260.png" alt="..." />
      </div>
      <button className="handle right-handle">
        <div className="text">&#129090;</div>
      </button>
    </div>
  );
}

export default Carousel
